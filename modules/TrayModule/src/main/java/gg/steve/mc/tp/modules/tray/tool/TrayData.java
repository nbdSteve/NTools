...
public class TrayData implements ToolData {
    ...
    public void onBlockBreak(BlockBreakEvent event, PlayerTool tool) {
        List<Block> blocks = CubeUtil.getCube(event.getPlayer(), event.getBlock(), tool.getRadius(), TrayModule.moduleId, true);
    ...
    @Override
    public void onInteract(PlayerInteractEvent playerInteractEvent, PlayerTool tool) {
