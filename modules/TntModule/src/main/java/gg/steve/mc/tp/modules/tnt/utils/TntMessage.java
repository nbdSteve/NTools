...
public enum TntMessage {
    ...
    TntMessage(String path, String... placeholders) {
        ...
        this.actionBar = FileManager.get(TntModule.moduleConfigId).getBoolean("messages." + this.path + ".action-bar");
    }

    public void message(Player receiver, String... replacements) {
        List<String> data = Arrays.asList(replacements);
        if (this.actionBar) {
            for (String line : FileManager.get(TntModule.moduleConfigId).getStringList("messages." + this.path + ".text")) {
